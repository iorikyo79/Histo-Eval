...
3. [ ] DATA LOADER: 이미지 쌍 목록 파싱 (JSON)
   - 테스트 파일: `tests/test_data_loader_json.py`
   - 테스트(긍정): 샘플 JSON(`[ {"image_pair_id":..., "source_path":..., "target_path":...} ]`) 파싱 성공.
   - 테스트(부정): 필수 필드 누락/타입 불일치 시 `ValueError`.

4. [ ] DATA LOADER: 이미지 로드(파일 존재/형식 검증)
   - 테스트 파일: `tests/test_image_io.py`
   - 테스트(긍정): 작은 샘플 이미지를 로드하여 `np.ndarray` 반환, `dtype`는 `uint8` 또는 `float32`.
   - 테스트(부정): 존재하지 않는 경로/손상 파일에서 `FileNotFoundError`/`ValueError`.
   - 수락 기준: 로딩 옵션(그레이스케일/컬러) 플래그에 따른 채널 수 일치.

5. [ ] PIPELINE: 파이프라인 모듈 인터페이스
...
17. [ ] PIPELINE C/D: 성능 개선 및 에지케이스 테스트
	- 테스트: 다양한 크기/밝기 이미지에 대해 안정적 동작.

18. [ ] PARALLEL: 멀티프로세싱 실험 실행(옵션)
	- 테스트: `multiprocessing`을 이용한 간단한 배치 처리 구현 테스트.

19. [ ] DOCS: `README.md` 초안 작성 및 실행 예시 추가
