"""
Pipeline A 동작 검증 스크립트
실제 이미지를 로드하고 Pipeline A를 적용하여 결과를 확인하고 저장합니다.
"""
import sys
...
def verify_pipeline_a(save_outputs: bool = True):
    """Pipeline A의 동작을 실제 이미지로 검증
    ...
    print("=" * 70)
    print("Pipeline A 동작 검증 시작")
    print("=" * 70)
    
    # Output 폴더 생성
    output_dir = project_root / "output" / "pipeline_a"
    if save_outputs:
    ...
    # 2. 각 이미지 쌍에 대해 Pipeline A 적용
    print("\n2. 각 이미지에 Pipeline A 적용:")
    print("-" * 70)
    ...
    for idx, pair in enumerate(pairs, 1):
        ...
        try:
            ...
            source_processed = process(source_img)
            ...
            if all_checks_passed:
                ...
                # 처리된 이미지 저장
                if save_outputs:
                    output_filename = f"{pair_id}_source_processed.png"
        ...
        try:
            ...
            target_processed = process(target_img)
            ...
            if all_checks_passed:
                ...
                # 처리된 이미지 저장
                if save_outputs:
                    output_filename = f"{pair_id}_target_processed.png"
                    output_path = output_dir / output_filename
    ...
    print("\n" + "=" * 70)
    if all_passed:
        print("✓ Pipeline A 검증 성공: 모든 이미지가 정상적으로 처리되었습니다.")
        if save_outputs:
            print(f"✓ 처리된 이미지가 {output_dir}에 저장되었습니다.")
        print("=" * 70)
        return True
    else:
        print("✗ Pipeline A 검증 실패: 일부 이미지 처리에 문제가 있습니다.")
        print("=" * 70)
        return False
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Pipeline A 동작 검증 스크립트")
    parser.add_argument("--no-save", action="store_true", 
                        help="처리된 이미지를 저장하지 않음")
    args = parser.parse_args()
    
    success = verify_pipeline_a(save_outputs=not args.no_save)
