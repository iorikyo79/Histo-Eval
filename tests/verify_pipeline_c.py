"""
Pipeline C 동작 검증 스크립트
실제 이미지를 로드하고 Pipeline C를 적용하여 결과를 확인하고 저장합니다.
Pipeline C는 eosin 채널(세포질)을 추출합니다.
"""
import sys
...
def verify_pipeline_c(save_outputs: bool = True):
    """Pipeline C의 동작을 실제 이미지로 검증
    ...
    print("=" * 70)
    print("Pipeline C 동작 검증 시작")
    print("=" * 70)
    
    # Output 폴더 생성
    output_dir = None
    if save_outputs:
        output_dir = project_root / "output" / "pipeline_c"
    ...
    print("\n" + "=" * 70)
    if all_passed:
        print("✓ Pipeline C 검증 성공: 모든 이미지가 정상적으로 처리되었습니다.")
        if save_outputs:
            print(f"✓ 결과 저장 위치: {output_dir}")
        print("=" * 70)
        return True
    else:
        print("✗ Pipeline C 검증 실패: 일부 이미지 처리에 문제가 있습니다.")
        print("=" * 70)
        return False
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Pipeline C 동작 검증 스크립트")
    parser.add_argument("--no-save", action="store_true", 
                        help="처리된 이미지를 저장하지 않음")
    args = parser.parse_args()
    
    success = verify_pipeline_c(save_outputs=not args.no_save)
